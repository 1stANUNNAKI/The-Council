...
# Build the replacement for buildGrandIraqMap & buildTwoRiversOfMesopotamia & buildDiverseContinents
replacement_terrain_code = f'''    /* -------------------------------------------------------------------------
...
# Find boundaries of buildGrandIraqMap and buildTwoRiversOfMesopotamia in content
pattern_terrain = re.compile(r'/\* -+[\s\S]+?function buildGrandIraqMap\(\) \{[\s\S]+?riverLines\.push\(tigrisMesh, euphratesMesh, shattMesh\);\s*\}', re.MULTILINE)
assert pattern_terrain.search(content), "Could not find buildGrandIraqMap & buildTwoRivers pattern!"

content = pattern_terrain.sub(replacement_terrain_code, content)
open(html_path, 'w', encoding='utf-8').write(content)
print("Successfully replaced buildGrandIraqMap & buildTwoRiversOfMesopotamia with 198-vertex authentic Iraq cradle!")
